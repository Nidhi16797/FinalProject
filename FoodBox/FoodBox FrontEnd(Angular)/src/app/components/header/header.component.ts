...
export class HeaderComponent implements OnInit {
  ...
  adminloginSubmit(){
    if(this.adminloginData.username.trim()==''||this.adminloginData.username==null){
      this.error="Enter Username";
      return
    }
    if(this.adminloginData.password.trim()==''||this.adminloginData.password==null){
    ...
    this.adminService.adminLogin(this.adminloginData).subscribe(data=>{
      this.auth=data;
      if(this.auth==true){
        this.logService.sendAdmin(this.adminloginData.username);
        let ref = document.getElementById('Acancel')
        ref?.click();
        this.formValue.reset();
